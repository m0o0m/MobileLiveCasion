package HistoryRoad {
	 ...
	 * 轮盘大小 显示结果
	*/
	public class BigOrSmallView extends RoadPosView {
		
		/*
		 * 轮盘 大小显示结果
		 @ number 结果索引
		*/
		public function BigOrSmallView(number:int) {
			SetRouleeteAtrrribute();
			if(roadResult==null){
				return;
			}
			gotoAndStop (roadResult[number][1]);
			_x = 2;
			_y = 2;
			maxRow = 7;
			_width =27.7;
		...
		*/
		public override function RoadPosition(rpv:RoadPosView,number:int,arr:Array=null):void{
			lastNum=roadResult[number][1];
			if(rpv==null){//最后一个值为空时，位置为默认位置
				x=_x;
				y=_y;
				l_startx=_x;
			}else{
				if(rpv.lastNum!=roadResult[number][1]){
