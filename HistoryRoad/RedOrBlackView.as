package HistoryRoad {
	 ...
	 * 轮盘 红黑显示结果
	*/
	public class RedOrBlackView extends RoadPosView {
		
		/*
		 * 轮盘 黑红显示结果
		 @ number 结果索引
		*/
		public function RedOrBlackView(number:int) {
			SetRouleeteAtrrribute();
			if(roadResult==null){
				return;
			}
			gotoAndStop (roadResult[number][0]);
			_x = 2;
			_y = 3;
			maxRow = 7;
			_width =23;
		...
		*/
		public override function RoadPosition(rpv:RoadPosView,number:int,arr:Array=null):void{
			lastNum=roadResult[number][0];
			if(rpv==null){//最后一个值为空时，位置为默认位置
				x=_x;
				y=_y;
				l_startx=_x;
			}else{
				if(rpv.lastNum!=roadResult[number][0]){
